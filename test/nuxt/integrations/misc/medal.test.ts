...
import { mockKickClip, mockKickVod, mockOEmbed, mockTwitchClip, mockTwitchVod } from '~~/test/mocks'
...
const provider = new MedalProvider()

describe('integrations/misc/providers/medal', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    provider.clearCache()
  })
  ...
  it.each([
    ...
    await expect(provider.getClip(url)).rejects.toThrow(`[Medal]: Invalid video URL (${url}).`)
  })
